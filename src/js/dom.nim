...
type
  EventTarget* {.importc.} = ref object of RootObj
    onbeforeinput*: proc (event: Event) {.closure.}
    oninput*: proc (event: Event) {.closure.}
  ...
  Clipboard* {.importc.} = ref object of EventTarget
  ...
  Navigator* {.importc.} = ref object of RootObj
    ...
    clipboard*: Clipboard ## https://developer.mozilla.org/en-US/docs/Web/API/Navigator/clipboard
    cookieEnabled*: bool
...
# Clipboard "methods"
proc writeText*(c: Clipboard, newClipText: cstring)
